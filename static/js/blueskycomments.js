function loadBlueskyComments(blueskyUser, commentId) {
  document.addEventListener("DOMContentLoaded", function () {
    const commentList = document.getElementById("bluesky-comments-list");
    commentList.innerHTML = "Loading comments...";

    fetch(
      `https://public.api.bsky.app/xrpc/app.bsky.feed.getPostThread?uri=at://${blueskyUser}/app.bsky.feed.post/${commentId}&depth=10`
    )
      .then((response) => response.json())
      .then((data) => {
        const replies = data.thread.replies || [];
        console.log("Comments:", replies);
        //replies.sort(sortByDate); // Sort by date
        replies.sort(sortByLike); // Sort by likes
